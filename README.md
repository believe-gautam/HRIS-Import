...
A small Django application that analyzes an HRIS CSV before any employee or reporting data is written. It shows source and accepted row totals, row-level errors, roots, direct-report counts, and exact reporting-cycle membership.
...
- Rendering tens of thousands of errors as one HTML page can be large even though analysis is linear. Pagination or a downloadable error CSV would be the next UI improvement.
...
1. Upload `examples/sample_hris.csv`; point out the totals, valid roots/manager counts, missing-manager error, two-person cycle, and excluded cycle follower.
